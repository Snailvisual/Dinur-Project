...
export default function TiktokAnalisaPage() {
  ...
  useEffect(() => {
    if (platform === "Instagram") {
      window.location.href = "/ig-analisa";
    }
  }, [platform]);

  // Ambil data content plan TikTok/Instagram dari localStorage sesuai platform
  const getPostedContentPlanRows = () => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem(
        platform === "TikTok" ? "tiktokContentPlanRows" : "contentPlanRows"
      );
      if (saved) {
        try {
  ...
  const mergeRows = (planRows: any[], analyticRows: any[]) => {
    return planRows.map(plan => {
      ...
      return {
        ...
        jenis: plan.jenis,
        avgWatchTime: found?.avgWatchTime || "",
        views: found?.views || "",
        like: found?.like || "",
  ...
  // State utama hanya rows, persist pakai tiktokAnalisaRows
  const [rows, setRows] = useState<any[]>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("tiktokAnalisaRows");
      if (saved) {
    ...
    if (postedRows.length > 0) return postedRows.map(plan => ({ ...plan, avgWatchTime: "", views: "", like: "", comment: "", share: "", save: "", er: "" }));
    return [initialRow];
  });
  // Kolom: tanggal, headline, jenis, reach, views, like, comment, share, save, er
  const [colWidths, setColWidths] = useState<number[]>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("tiktokAnalisaColWidths");
      if (saved) return JSON.parse(saved);
    }
    return [120, 180, 140, 110, 90, 90, 90, 90, 90, 90]; // avgWatchTime lebih lebar
  });
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      localStorage.setItem("tiktokAnalisaRows", JSON.stringify(rows));
      localStorage.setItem("tiktokAnalisaColWidths", JSON.stringify(colWidths));
    }
  }, [rows, colWidths]);
  ...
  const deleteAllRows = () => {
    setRows([]);
    if (typeof window !== 'undefined') {
      localStorage.removeItem("tiktokAnalisaRows");
    }
  };
  ...
  const calculateER = (row: any) => {
    ...
    const er = ((like + comment + share + save) / views) * 100;
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const analyticRows = JSON.parse(localStorage.getItem("tiktokAnalisaRows") || "[]");
      const planRows = getPostedContentPlanRows();
      if (planRows.length > 0) {
        setRows(mergeRows(planRows, analyticRows));
      } else {
        setRows([initialRow]);
      }
    }
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-3xl font-bold mb-4 text-[#56ad9c]">TikTok Content Insight</h1>
            ...
            <table className="table-auto w-max min-w-full bg-white rounded-lg text-sm border border-gray-300" style={{ fontSize: 13 }}>
              ...
              <thead>
                <tr className="bg-[#56ad9c] text-white select-none text-sm border-b border-gray-300" style={{ fontSize: 13 }}>
                  {[
                    "Tanggal Posting","Headline Content","Jenis Content","Avg Watch Time","Views","Like","Comment","Share","Save","ER"
                  ].map((col, idx) => (
                    <th
                      key={col}
                    ...
                    <tr key={idx} className="text-black text-sm border-b border-gray-200 last:border-b-0" style={{ fontSize: 13 }}>
                      ...
                      <td className="border px-2 py-2 text-center text-sm border-gray-200 bg-white" style={{ fontSize: 13 }}>
                        <input
                          ...
                          value={row.avgWatchTime}
                          onChange={e => handleAnalisaChange(realIdx, "avgWatchTime", e.target.value)}
                          style={{ fontSize: 13 }}
                        />
                      </td>
                      {/* Views */}
                      ...
                      <td className="border px-2 py-2 text-center text-sm border-gray-200 bg-white" style={{ fontSize: 13 }}>
                        <input
                          ...
                          readOnly
                          style={{ fontSize: 13 }}
                          title="ER = (Like + Comment + Share + Save) / Views x 100%"
                        />
                      </td>
                    </tr>
                  );
